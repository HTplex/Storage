...
public class FuzzyEngine
{
	...
	//Hashtables
	private Hashtable<String, LinguisticVariable> lvHash = null;
	private Hashtable<String, Integer> controlHash = null;
	private Hashtable<String, Hedge> hedgesHash = null;
	private Hashtable<String, LinguisticVariable> labelWeightsHash = null;

	//Nesting stack
	private Stack<StackElement> nestingStack = null;
	 ...
	 */
	private String parseExpression(java.util.StringTokenizer tokens,FuzzyRule rule) throws RulesParsingException{
		...
		//Hedges buffer
		Stack<Hedge> hedgesBuffer = new Stack<Hedge>();
...
//						LABEL:STORE_LABEL --------------------------------------------------------------------------------------------
					case STORE_LABEL:
						...
						case IF:
							...
							{
								//retrive WEIGHT LV
								LinguisticVariable tempLV = lvHash.get("weight");
...
//					EVALUATION:COMPLETE_EVALUATION -------------------------------------------------------------------------------
					if(engineSubState==COMPLETE_EVALUATION){
						...
						if(rule!=null){
							...
							Vector<Hedge> hVector = new Vector<Hedge>();
							while(!hedgesBuffer.empty())
								hVector.addElement(hedgesBuffer.pop());
...
//					EXECUTION: COMPLETE_EXECUTION -------------------------------------------------------------------------------
	...
	public void reset()
	{
		//Reset fired rules
		flagRuleFired = false;
		for (Enumeration<LinguisticVariable> en = lvHash.elements() ; en.hasMoreElements() ;)
		{
			((LinguisticVariable)en.nextElement()).reset();
		}
		//Reset labels' weights changes
		for (Enumeration<LinguisticVariable> en = labelWeightsHash.elements() ; en.hasMoreElements() ;)
		{
			en.nextElement().reset();
		}
	}
	/**
	 * Constructor.
	...
	public FuzzyEngine(){
		...
		//Create LV hashtable
		lvHash = new Hashtable<String, LinguisticVariable>();


		//Create nesting stack
		nestingStack = new Stack<StackElement>();

		//Initialize hedges hash
		hedgesHash = new Hashtable<String, Hedge>();
//		addHedge(new HedgeNot());
//		addHedge(new HedgeVery());
//		addHedge(new HedgeSomewhat());

		//Create labels weights hash
		labelWeightsHash = new Hashtable<String, LinguisticVariable>();

		//Init. control controlHash hashtable
		controlHash = new Hashtable<String, Integer>();
