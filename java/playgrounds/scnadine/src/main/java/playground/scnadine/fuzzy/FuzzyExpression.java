...
public class FuzzyExpression 
{
	...
	private Vector<Hedge> hedges = null;
	...
	public double evaluateExpression() throws EvaluationException 
	{
		...
		for (Enumeration<Hedge> en = hedges.elements() ; en.hasMoreElements() ;){
			result = en.nextElement().hedgeIt(result);
		}
	 ...
	 */
	public FuzzyExpression(LinguisticVariable lVariable, String mVariable, Vector<Hedge> hVector, String textRepresentation) 
	{
		LV = lVariable;
		MF = mVariable;
		hedges = hVector;
