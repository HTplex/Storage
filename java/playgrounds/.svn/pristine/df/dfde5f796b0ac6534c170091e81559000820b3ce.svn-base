...
import playground.scnadine.converters.osmCore.OsmParser.OsmNode;
import playground.scnadine.converters.osmCore.OsmParser.OsmRelation;
import playground.scnadine.converters.osmCore.OsmParser.OsmRelationMember;
import playground.scnadine.converters.osmCore.OsmParser.OsmRelationMemberType;
...
public class Osm2TransitLines {
	...
	/* package */final Map<Long, OsmRelation> relations = new HashMap<Long, OsmRelation>();
	/* package */final Map<Long, OsmWay> ways = new HashMap<Long, OsmWay>();
	/* package */final Map<Long, OsmNode> nodes = new HashMap<Long, OsmNode>();
	/* package */final Map<Long, OsmNode> stopNodes = new HashMap<Long, OsmNode>();
	...
	/* package */final Map<Long, List<Id<Link>>> waysToLinks_DirA = new HashMap<Long, List<Id<Link>>>();
	/* package */final Map<Long, List<Id<Link>>> waysToLinks_DirB = new HashMap<Long, List<Id<Link>>>();

	public Osm2TransitLines(final TransitSchedule schedule,
	...
	private TransitRouteWaysAndStopsCollector collectTransitRouteWays(final String filename, final Map<Long, OsmRelation> relations,final Set<Long> wayIds, final Set<Long> nodeIds,final Set<Long> stopNodeIds) {
	...
	private WaysCollector collectWays(final String filename, final Set<Long> wayIds, final Set<Long> nodeIds, final Map<Long, OsmWay> ways) {
		OsmParser parser = new OsmParser();
	...
	private NodesCollector collectNodes(final String filename, final Set<Long> nodeIds, final Set<Long> stopNodeIds, final Map<Long, OsmNode> nodes, final Map<Long, OsmNode> stopNodes) {
		OsmParser parser = new OsmParser();
	...
	private void createNetworkOfTransitRoutes(final Map<Long, OsmNode> nodes, final Map<Long, OsmWay> ways,	final Map<Long, List<Id<Link>>> waysToLinks_DirA,	final Map<Long, List<Id<Link>>> waysToLinks_DirB, final Network network) {
	...
	private void createSchedule(final Map<Long, OsmRelation> relations,	final Map<Long, OsmNode> stopNodes, final Map<Long, OsmWay> ways, final Map<Long, OsmNode> nodes, Map<Long, List<Id<Link>>> waysToLinks_DirA,	Map<Long, List<Id<Link>>> waysToLinks_DirB,
...
//		creator.createLines();
	...
	OsmRelationHandler {
		...
		public TransitRouteWaysAndStopsCollector(final Map<Long, OsmRelation> relations, final Set<Long> wayIds, final Set<Long> nodeIds, final Set<Long> stopNodeIds) {
		...
		public void handleRelation(OsmRelation relation) {
			...
			if (this.filter.matches(relation.tags)) {
				...
				for (OsmRelationMember member : relation.members) {
					...
					} else if (member.type == OsmRelationMemberType.NODE) {
	...
	private static class NodesCollector implements OsmNodeHandler {
		...
		private final Map<Long, OsmNode> stopNodes;

		public NodesCollector(final Set<Long> nodeIds,
				final Set<Long> stopNodeIds, final Map<Long, OsmNode> nodes,
				final Map<Long, OsmNode> stopNodes) {
			this.nodeIds = nodeIds;
			this.stopNodeIds = stopNodeIds;
			this.nodes = nodes;
	...
	private static class NetworkCreator {
		...
		public NetworkCreator(final Map<Long, OsmNode> nodes,
				final Map<Long, OsmWay> ways,
	...
	private static class ScheduleCreator {
		...
		public ScheduleCreator(final Map<Long, OsmRelation> relations,
				final Map<Long, OsmNode> stopNodes,
				final Map<Long, OsmWay> ways, final Map<Long, OsmNode> nodes,
		...
		public void createStops() {
			for (OsmNode node : this.stopNodes.values()) {
		...
		public void createLines() {
			...
			for (OsmRelation relation : this.relations.values()) {
				...
				try {
					...
					List<OsmNode> relationStops = new ArrayList<OsmNode>();
					...
					for (OsmRelationMember member : relation.members) {
						...
						if (member.type == OsmRelationMemberType.RELATION) {
						...
						} else if (member.type == OsmRelationMemberType.WAY) {
					...
					List<Id<TransitStopFacility>> relationStopIds = new ArrayList<Id<TransitStopFacility>>();
					for (OsmNode node : relationStops) {
		...
		private List<Id<TransitStopFacility>> sortStops(List<Id<TransitStopFacility>> stops) {
			List<Id<TransitStopFacility>> sortedStops = new LinkedList<Id<TransitStopFacility>>();
		...
		private void sortWays(List<Long> sortedWays, List<Id<Link>> sortedLinks, List<Long> unsortedWays, Map<Long, OsmWay> relationWays) {
		...
		private void developWays(List<Long> sortedWays, List<Id<Link>> sortedLinks, Map<Long, OsmWay> relationWays, List<Long> loopWays, Map<Long, List<Long>> loopWayNodes, HashMap<Long, List<Long>> unprocessedNodesAndConnectedWays, long startNodeId) {
		...
		private void insertLoop(List<Long> sortedWays, List<Id<Link>> sortedLinks, Map<Long, OsmWay> relationWays, Map<Long, List<Long>> loopWayNodes, HashMap<Long, List<Long>> unprocessedNodesAndConnectedWays, List<Long> loopNodes, List<Long> forkNodes, long nodeId) {
		...
		private void developFork(List<Long> sortedWays, List<Id<Link>> sortedLinks, Map<Long, OsmWay> relationWays, Map<Long, List<Long>> loopWayNodes, HashMap<Long, List<Long>> unprocessedNodesAndConnectedWays, List<Long> loopNodes, List<Long> forkNodes, long nodeId) {
			...
			List<Id<Link>>sortedLinksBeforeFork = new ArrayList<Id<Link>>();
			List<Id<Link>>sortedLinksAfterFork = new ArrayList<Id<Link>>();
		...
		private long developWay(List<Long> sortedWays, List<Id<Link>> sortedLinks, Map<Long, OsmWay> relationWays, Map<Long, List<Long>> loopWayNodes, HashMap<Long, List<Long>> unprocessedNodesAndConnectedWays, List<Long> loopNodes, List<Long> forkNodes, long nodeId) {
		...
		private long developLoopWay(List<Long> sortedWays, List<Id<Link>> sortedLinks, Map<Long, OsmWay> relationWays, Map<Long, List<Long>> loopWayNodes, HashMap<Long, List<Long>> unprocessedNodesAndConnectedWays, long nodeId) {
		...
		private List<Id<Link>> translateWayToLinks(long wayId, Map<Long, OsmWay> relationWays, long startNodeId) {
			List<Id<Link>> linkIds = null;
			OsmWay way = relationWays.get(wayId);
		...
		private List<Id<Link>> translateLoopWayToLinks(long wayId, Map<Long, OsmWay> relationWays, long startNodeId, long endNodeId) {
		...
		private TransitStopFacility createStop(OsmNode node) {
