...
/* *********************************************************************** *
 * project: posdap.*
 * GenerateChessboardNetworkWithDiagonals.java
 *                                                                         *
 * *********************************************************************** *
 *                                                                         *
...
public class GenerateChessboardNetworkWithDiagonals {
	...
	public static void main(String[] args) {
		...
		if (!oneway) {
			...
			/*Generate eastward and westward links*/
			System.out.println("Generate northeast to southwest links");
			for (int i = 0; i < gridSize*(gridSize-1); i++) {	
				
				if (i % gridSize != 0) {					
					Node fromNode = network.getNodes().get(Id.create(i+"", Node.class));	
					Node toNode = network.getNodes().get(Id.create(i+gridSize-1, Node.class));
					Id<Link> linkIdE = Id.create(linkIdInt+"_E", Link.class);
					Id<Link> linkIdW = Id.create(linkIdInt+"_W", Link.class);
					Link linkE = network.getFactory().createLink(linkIdE, fromNode, toNode);
					Link linkW = network.getFactory().createLink(linkIdW, toNode, fromNode);
					double linkLength = CoordUtils.calcDistance(fromNode.getCoord(), toNode.getCoord()); 
					linkE.setLength(linkLength);
					linkE.setFreespeed(15);
					linkW.setLength(linkLength);
					linkW.setFreespeed(15);					
					network.addLink(linkE);
					network.addLink(linkW);
					linkIdInt++;
			...
			/*Generate eastward and westward links*/
			System.out.println("Generate southeast to northhwest links");
			for (int i = 0; i < gridSize*(gridSize-1); i++) {	
		...
		new NetworkWriter(scenario.getNetwork()).write(targetDir.toString()+"/chessboardWithDiagonals"+gridSize+"by"+gridSize+"network.xml");
